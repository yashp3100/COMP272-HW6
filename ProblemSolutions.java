...
public class ProblemSolutions {
    ...
    public static int lastBoulder(int[] boulders) {
        ...
        // Put the boulders to the max heap
        ...
        while (Queue.size() > 1) {
            int x = Queue.poll(); // 1st
            int y = Queue.poll(); // 2nd
            //positive difference
            if (x != y) {
                Queue.add(x - y);
            }
    ...
    public static ArrayList<String> showDuplicates(ArrayList<String> input) {
        ...
        //Get the strings that have more than one occurrence
        ArrayList<String> two = new ArrayList<>();
        for (Map.Entry<String, Integer> insert : count.entrySet()){
            if(insert.getValue() > 1){
                two.add(insert.getKey());
            }
        }
        ...
        return two;  // Make sure result is sorted in ascending order
    }
    ...
    public static ArrayList<String> pair(int[] input, int k) {
        ...
        //  YOUR CODE GOES HERE
        //
        return new ArrayList<>(duplicates);  // Make sure returned lists is sorted as indicated above
    }
}
